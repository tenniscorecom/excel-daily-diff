...
| **積み上げだけ0件になる** | 絞り込み条件（`PLAN_PREFIXES`・`KINDS`・追加の絞り込み）が実際の案件と合っていない | 積み上げ（新しく入ってきた案件）は先の日付を持つ。条件が厳しすぎると、増えた案件が全部対象外に落ちて積み上げだけ0になる。黒い画面の「条件に合う行 ◯件」が極端に少なくないか、「追加の絞り込み: …」が意図どおりかを確認する |
| 表の右のほうが全部空欄 | その日付のファイルがフォルダに無い | 空欄は「比べられなかった」という意味（`0` は「比べた結果0件」）。対象月の前／後の日付列も空欄になる（対象月は実行日から自動で決まる） |
| 集計が全部0になる | 条件に合う行が1件も無い | `PLAN_PREFIXES`（標準・上位）と `KINDS`（完了・予定）の書き方が元ファイルと一致しているかを確認する。`[INCLUDE_CONTAINS]` / `[INCLUDE_EXACT]` に書いた条件が厳しすぎないかも見る（黒い画面に「追加の絞り込み: …」として表示される）。絞り込み条件を変えると過去ぶんも再計算されるため、`集計.conditions.txt` が前回と違っていないかも合わせて確認する |
...
> このツール固有の例外は **`ExcelColumnNotFoundError` など comken 由来のものだけ**に整理されています。
> 旧バージョンにあった `ComparisonFileNotEnoughError` / `InvalidDateSettingError` /
> `InvalidMonthSettingError` / `InvalidFilePatternError` は、設定の自動化（対象月・期間は
> 実行時に決まる、ファイル名は `FILE_PATTERN` だけ）に伴って廃止されました。
