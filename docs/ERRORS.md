...
| エラー名・症状 | 意味 | 対処 |
|---|---|---|
| `ComparisonFileNotEnoughError` | 比べる相手のファイルが足りない（前日と当日の2つが必要） | `config.ini` の `INPUT_FOLDER` が正しいか確認する。フォルダの中のファイル名に `一覧_20260812.xlsx` のように日付が入っているかも確認する |
| `InvalidDateSettingError` | `config.ini` の `START_DATE` / `END_DATE` が日付として読めない | `2026-04-21` のように「年-月-日」をハイフンでつないだ形に直す（`2026/4/21` や `8月` は不可） |
...
| 集計が全部0になる | 条件に合う行が1件も無い | `config.ini` の `START_DATE` / `END_DATE` が元ファイルの日付と合っているか、`PLAN_PREFIXES`（標準・上位）と `KINDS`（完了・予定）の書き方が元ファイルと一致しているかを確認する。`[INCLUDE_CONTAINS]` / `[INCLUDE_EXACT]` に書いた条件が厳しすぎないかも見る（黒い画面に「追加の絞り込み: …」として表示される） |
