Attribute VB_Name = "DiffPipeline"
Option Explicit
...
' 入力フォルダ（ExportV3 の OUTPUT_FOLDER。このパイプラインでは読み取り元）から
...
' Python の [FILTER] セクション（= ExportV3.BuildFilterSQL）と同じ絞り込みを、
' 対象月を引数にして WHERE 句として組み立てる。ExportV3 の定数をそのまま参照するので、
